...
def init_colors():
    ...
    curses.init_pair(7, curses.COLOR_BLACK, 13) # 13 - pink
...
def draw_game_window(window):
    ...
    if game_board.is_game_over():
        go_title = " " + loc_lines[0] + " "
        ag_title = " Enter - " + loc_lines[1] + " "
        s_title = " S - " + loc_lines[6] + " "
        ...
        window.addstr(int(GAME_WINDOW_HEIGHT*.6), (GAME_WINDOW_WIDTH-len(s_title))//2, s_title, curses.color_pair(95))
...
def draw_status_window(window):
    ...
    window.addstr(1, 2, f"{loc_lines[3]}: {game_board.score}")
    window.addstr(2, 2, f"{loc_lines[4]}: {game_board.lines}")
    window.addstr(3, 2, f"{loc_lines[5]}: {game_board.level}")
...
def draw_help_window():
    ...
    window.addstr(1, 2, loc_lines[7] + " ← ↓ →")
    window.addstr(2, 2, loc_lines[8] + " space")
    window.addstr(3, 2, loc_lines[9] + " ↑")
    window.addstr(4, 2, loc_lines[10] + " p")
    window.addstr(5, 2, loc_lines[11] + " q")
...
if __name__ == "__main__":
    try:
        with open(LOC + '.loc') as file:
            loc_lines = [line.rstrip() for line in file]
...
print(loc_lines[6] + ":")
for i in range(10):
    print(str(i+1) + ": " + str(game_board.best_score[i]))
input()